...
#include "Buttons/ProfileButton.h"
#include <memory>
...
MenuComponents::MenuComponents(Driver* driver) : _driver{driver} {
    ...
    buttons.push_back(new PlayButton(250, 300, _driver));
    buttons.push_back(new EditorButton(250, 420, _driver));
    buttons.push_back(new QuitButton(250, 540));
    buttons.push_back(new ProfileButton(630, 20, this));
    profileMenu = std::make_shared<ProfileMenu>(100, 100, _driver);
}
...
void MenuComponents::openProfileMenu() {
    profileMenu->changeVisibility();
}