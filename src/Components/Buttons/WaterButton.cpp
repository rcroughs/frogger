...
WaterButton::WaterButton(int x, int y, Driver *driver) : x{x}, y{y}, driver{driver} {
...
void WaterButton::showConfigurationButtons() {
    ...
    for (int i=3; i<8; i++) {
        ...
        buttons.at(i)->changePosition(buttons.at(i)->getX(), (height - (height / 13.0f)) - (driver->getEditor()->getCurrentRow() * (height / 13.0)));
    }
}
